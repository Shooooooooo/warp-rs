...
fn on_a_pty(args: &[&str], hold_for: Duration) -> (String, Option<i32>) {
    ...
    // when that reaches end of file.
    let mut child = Command::new("script")
        .args(["-qec", &command])
        .arg(&log)
        .stdin(std::process::Stdio::piped())
        // Silenced, because `script` echoes the whole session to its own stdout
        // as well as recording it: inherited, that is twenty-eight megabytes of
        // escape sequences into the test log for one flight.
