...
/// What the box says about getting out of it, widest first: the first that fits
/// is the one drawn, the same way the panel picks a hint tier.
const FOOTERS: [&str; 2] = [
...
pub fn draw(screen: &mut Screen, menu: &Menu) {
    ...
    // Both runs of the box's horizontal, which differ only in what they have to
    // reach across: the corners' spans the two frame characters, the rule under
    // the title spans the body between them.
...
mod tests {
    ...
    fn the_box_is_drawn_in_one_colour_and_what_is_in_it_in_their_own() {
        ...
        // whole run and gives it to every glyph in it that is not a space.
        let (cols, rows) = (120usize, 34usize);
        let ships = models::models();
        ...
        for mode in [ColorMode::Truecolor, ColorMode::Ansi256] {
            ...
            // FRAME snapped to a palette entry, which this module has no way to
            // spell.
    ...
    fn the_box_always_closes_and_always_says_how_to_leave_it() {
        ...
        for rows in MIN_ROWS..=40 {
            for cols in [MIN_COLS, 34, 40, 48, 63, 80, 200] {
                ...
                // Verbatim, not merely present: a footer clipped to `ESC keep
                // the one you hav` still contains the word.
                assert!(
    ...
    fn a_short_box_windows_the_list_and_says_that_it_did() {
        // Shedding a blurb can go quietly.
        let menu = Menu::new(0);
    ...
    fn a_box_with_the_room_finishes_every_sentence_in_it() {
        ...
        // terminal size, the widest included.
